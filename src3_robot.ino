...
#define KEY7 16716015 //Camera Move mode (FF10EF)
#define KEY9 16734885 //(FF5AA5)
...
#define camDelta 10 //andle to rotate camera / servo step


unsigned int carSpeed_rocker = 250;
#define PIN_Servo 3
#define PIN_Servo_Cam 8
Servo servo;             //  Create a DC motor drive object
Servo servo_cam;             //  Create a DC motor drive object
...
enum MOTIONMODE
{
  ...
  RIGHT_FORWARD,
  RIGHT_BACK,
  //CameraMove,            /*Move servo for camera*/
  CM_U,
  CM_D, 
  CM_L,
  CM_R,  
...
*/
void ServoControl(uint8_t angleSetting, uint8_t pin = PIN_Servo)
{
  if (angleSetting > 175)
  {
    angleSetting = 175;
...
uint8_t ServoPosition(uint8_t pin)
{
  uint8_t cur_pos;
  servo.attach(pin);
  cur_pos = servo.read();
...
void getIRData(void)
{
  if (irrecv.decode(&results))
  {
    ...
    case KEY7:
      func_mode = IRremote;
      mov_mode = CM_L;
      break; /*Move camera left*/
    case KEY9:
      func_mode = IRremote;
      mov_mode = CM_R;
      break; /*Move camera right*/
    default:
      break;
    }
    irrecv.resume();
...
void irremote_mode(void)
{
  if (func_mode == IRremote)
  {
    ...
    case STOP:
      ...
      break;
    case CM_U:
      camera_move_mode(camDelta, PIN_Servo_Cam);
      break;
    case CM_D:
      camera_move_mode(-camDelta, PIN_Servo_Cam);
      break;
    case CM_L:
      camera_move_mode(camDelta, PIN_Servo);
      break;
    case CM_R:
      camera_move_mode(-camDelta, PIN_Servo);
      break;
...
/*
  Camera Move Mode
*/
void camera_move_mode(uint8_t ang_val, uint8_t pin)
{
  uint8_t cur_pos;
  cur_pos = ServoPosition(pin);
  ServoControl(cur_pos+ang_val, pin);
}
/*
  Line Teacking Mode
*/
